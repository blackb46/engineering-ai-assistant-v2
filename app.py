...
with col_db:
    if db_info["success"]:
        ...
        <div class="status-ok">
            ...
            {size_mb} MB synced from Drive{cache_note}<br>
            <small>Last sync: {sync_ts}</small>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
...
if not db_info["success"]:
    st.error(
        ...
        "Check that DRIVE_DB_FOLDER_ID is set in Streamlit secrets and that "
        "build_corpus.py has been run in Google Colab."
    )
    st.stop()
